"""
Extract Fast CAT event results from AKC website for 2021-2022 events.
Combines logic from extract_fcat_results_2025.py and extract_dog_data.py.
"""
...
def sanitize_for_excel(text):
    ...
    if not isinstance(text, str):
        return text
    import re
    # Remove control characters (0x00-0x1F except tab/newline/return)
    text = re.sub(r"[\x00-\x08\x0B-\x0C\x0E-\x1F]", "", text)
    # Remove arrow characters
    text = text.replace(chr(0x2193), "").replace(chr(0x2191), "")
    text = text.replace(chr(0x2192), "").replace(chr(0x2190), "")
    text = text.replace(chr(0x2194), "").replace(chr(0x2195), "")
    return text
...
def scrape_event_results(url, event_info):
    """Scrape results from a single event URL including dog_id extraction"""
    try:
        ...
        while i < len(all_divs):
            ...
            if link:
                ...
                results.append({
                    ...
                    'Dog Name': sanitize_for_excel(dog_name),
                    'Dog ID': dog_id,
                    'Breed': sanitize_for_excel(breed),
                    'Owner': sanitize_for_excel(owner),
                    'Speed (MPH)': speed,
                    'Points': points,
                    'Event URL': url,
                    'Dog Profile URL': dog_url,
                    'Dog Points URL': build_points_url(dog_id) if dog_id else ''
                })
...
def main(test_mode=False):
    # Input CSV file
    csv_file = r'2021-2022 FastCAT Events.csv'
    
    print("="*80)
    print("Starting Fast CAT results extraction (2021-2022 Events)...")
    print("="*80)
    ...
    if test_mode:
        # Process only the first event for testing
        events = events[:1]
        print(f"TEST MODE: Processing only 1 event for testing\n")
    else:
    ...
    for idx, event in enumerate(events, 1):
        ...
        if not test_mode or idx < len(events):
            time.sleep(1)
        
        # Progress update every 100 events
        if not test_mode and idx % 100 == 0:
    ...
    if all_results:
        ...
        if test_mode:
            output_file = f"FastCAT_Results_2021_2022_TEST_{timestamp}.xlsx"
        else:
            output_file = f"FastCAT_Results_2021_2022_Complete_{timestamp}.xlsx"
...
if __name__ == "__main__":
    # Set to True to test with one event first, False to process all events
    TEST_MODE = False
